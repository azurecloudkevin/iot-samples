...
class RobotMotion:
    ...
    def move(self):
        ...
        if self.pose:
            newPose = []
            for i in range(len(list(self.pose))):
                newPose.append((self.pose[i] - self.lastPose[i]) * self.step + self.lastPose[i])
            self.setPose(newPose)

        # if self.pose_sequence.qsize > 0:
        #     nextpose = self.pose_sequence.get_nowait()
        #     print("Dequeued point is ", nextpose)
        #     for i in range(len(list(self.pose))):
        #         self.pose[i] = nextpose
    ...
    async def startAnimating(self):
        ...
        while self.animating:
            self.move()
            # print("animating...")
            await asyncio.sleep(self.ms / 100000)
